(function(win, lib) {
    ...
    function setViewport() {
        ...
        if (hasMetaEl) {
            ...
            if (isIPhone) {
                hasMetaEl.setAttribute('content', 'width=device-width, initial-scale=' + scale +
                    ', user-scalable=no');
            }
            // target-densitydpi 目标设备密度等级，默认值medium-dpi，我们的目标是css中的1px会等于物理像素中的1px，故使用target-densitydpi=device-dpi
            else {
                hasMetaEl.setAttribute('content', 'width=device-width, initial-scale=' + scale + ', maximum-scale=' +
...
$(function() {
    window['adaptive'].desinWidth =2400;
